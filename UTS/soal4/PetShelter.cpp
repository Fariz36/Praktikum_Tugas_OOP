...
PetShelter<T>::PetShelter(string shelterName) {
    for (int i = 0; i < MAX_SHELTER_CAPACITY; i++) {
        pets[i] = nullptr; // Initialize all pet pointers to nullptr
    }
    this->shelterName = shelterName;
    dogCapacity = 8; // Maximum capacity for dogs
    catCapacity = 12; // Maximum capacity for cats
    petCount = 0; // Total number of pets in the shelter
    currentDogs = 0; // Current number of dogs in the shelter
    currentCats = 0; // Current number of cats in the shelter
}
/* Default constructor:
...
PetShelter<T>::PetShelter(string shelterName, int dogCapacity, int catCapacity) {
    ...
    this->shelterName = shelterName;
    this->dogCapacity = dogCapacity; // Maximum capacity for dogs
    this->catCapacity = catCapacity; // Maximum capacity for has bee
    petCount = 0; // Total number of pets in the shelter
    currentDogs = 0; // Current number of dogs in the shelter
    currentCats = 0; // Current number of cats in the shelter
}
...
T* PetShelter<T>::findPet(const string &petID) const {
    for (int i = 0; i < MAX_SHELTER_CAPACITY; i++) {
        if (pets[i] == nullptr) continue; // Skip empty slots
...
void PetShelter<T>::removePet(const string &petID) {
    for (int i = 0; i < MAX_SHELTER_CAPACITY; i++) {
        ...
        if (pets[i]->getId() == petID) {
            ...
            pets[i] = nullptr; // Set the pointer to nullptr

            return; // Exit the function after removing the pet
        }
    }
...
// int main() {
//     PetShelter<Pet> PS("nigga", 2, 2);
//     PS.addPet("A1", 1, true, "cihuahua");
//     PS.addPet("A2", 1, true, "golden retriver");
//     PS.addPet("A3", 1, false, "hitam");
//     PS.addPet("A4", 1, true, "bulldog");

//     cout << PS.calculateCost() << endl;
